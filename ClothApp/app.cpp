...
// Animation
static const int g_fps = 300; // frames per second  | 60
static const int g_iter = 10; // iterations per time step | 10
static const int g_frame_time = 1000 / g_fps;
...
static void initShaders() {
  ...
  ifs.open("./shaders/basic.vshader", std::ifstream::in);
  basic_vert.compile(ifs);
  ifs.close();

  ifs.open("./shaders/phong.fshader", std::ifstream::in);
  phong_frag.compile(ifs);
  ifs.close();

  ifs.open("./shaders/pick.fshader", std::ifstream::in);
  pick_frag.compile(ifs);
  ifs.close();
...
static void demo_hang() {
  ...
  massSpringBuilder.uniformGrid(SystemParam::n, SystemParam::h, SystemParam::r,
                                SystemParam::k, SystemParam::m, SystemParam::a,
                                SystemParam::g);
...
static void animateCloth(int value) {
  ...
  if (currenttime - timebase > 1000) {
    char title[256];
    sprintf(title, "Cloth App FPS: %4.2f",
